...
public class AESUtil {
    ...
    public static final String DEFAULT_PWD = "abcdefghijklmnop";//加密和解密的key要一致
    ...
    */
    public static File encryptAPKFile(File srcAPKfile, File dstApkFile) throws Exception {
        ...
        ZipUtil.unZip(srcAPKfile, dstApkFile);//解压apk文件到指定目录下
        //获得所有的dex （需要处理分包情况）
        File[] dexFiles = dstApkFile.listFiles(new FilenameFilter() {
            @Override
            public boolean accept(File file, String s) {
                return s.endsWith(".dex");//提取出所有dex文件
            }
        });
